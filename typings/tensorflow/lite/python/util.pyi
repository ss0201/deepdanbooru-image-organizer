...
def get_tensors_from_tensor_names(graph, tensor_names): # -> list[Unknown]:
...
def get_grappler_config(optimizers_list): # -> ConfigProto:
...
def run_graph_optimizations(graph_def, input_arrays, output_arrays, config, graph=...):
...
def freeze_graph(sess, input_tensors, output_tensors): # -> GraphDef | Any:
...
def build_debug_info_func(original_graph): # -> (original_nodes: Unknown) -> (GraphDebugInfo | None):
...
def convert_debug_info_func(saved_debug_info): # -> (original_nodes: Unknown) -> (GraphDebugInfo | None):
  """Returns a method to retrieve the `GraphDebugInfo` from the original graph.
...
def get_quantize_opcode_idx(model): # -> list[Unknown]:
  """Returns the quantize op idx."""
  ...

def get_dequantize_opcode_idx(model): # -> list[Unknown]:
  """Returns the quantize op idx."""
  ...
...
def get_sparsity_modes(model_object): # -> list[Unknown]:
