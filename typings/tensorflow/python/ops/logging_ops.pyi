...
def Print(input_, data, message=..., first_n=..., summarize=..., name=...):
...
@tf_export("print")
@dispatch.add_dispatch_support
def print_v2(*inputs, **kwargs): # -> None:
...
def histogram_summary(tag, values, collections=..., name=...):
...
def image_summary(tag, tensor, max_images=..., collections=..., name=...):
...
def audio_summary(tag, tensor, sample_rate, max_outputs=..., collections=..., name=...):
...
def merge_summary(inputs, collections=..., name=...):
  """Merges summaries.
...
def scalar_summary(tags, values, collections=..., name=...):
