...
class LossesContainer(Container):
  ...
  def __call__(self, y_true, y_pred, sample_weight=..., regularization_losses=...): # -> defaultdict[Unknown, Unknown] | Any | list[Unknown] | ObjectProxy | Tensor:
...
class MetricsContainer(Container):
  ...
  @property
  def unweighted_metrics(self): # -> list[None] | None:
    """Metrics in this container that should not be passed `sample_weight`."""
    ...
  
  @property
  def weighted_metrics(self): # -> list[None] | None:
...
def create_pseudo_output_names(outputs): # -> list[Unknown]:
  """Create pseudo output names for a subclassed Model."""
  ...

def create_pseudo_input_names(inputs): # -> list[Unknown]:
  """Create pseudo input names for a subclassed Model."""
  ...

def map_to_output_names(y_pred, output_names, struct): # -> list[Unknown | None] | None:
...
def map_missing_dict_keys(y_pred, struct): # -> dict[Unknown, Unknown]:
  """Replaces missing dict keys in `struct` with `None` placeholders."""
  ...

def match_dtype_and_rank(y_t, y_p, sw): # -> tuple[Unknown | SparseTensor | IndexedSlices | Tensor | Any, Unknown, Unknown | SparseTensor | IndexedSlices | Tensor | Any]:
...
def apply_mask(y_p, sw, mask): # -> SparseTensor | IndexedSlices | Tensor | Any:
