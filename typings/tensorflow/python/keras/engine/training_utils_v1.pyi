...
def has_symbolic_tensors(ls): # -> bool | Any:
  ...

def has_tensors(ls): # -> bool | Any:
...
def get_metric_function(metric, output_shape=..., loss_fn=...): # -> Any | ((y_true: Unknown, y_pred: Unknown, threshold: float = 0.5) -> Unknown) | ((y_true: Unknown, y_pred: Unknown) -> (Unknown | SparseTensor | IndexedSlices | Tensor | Any)) | ((y_true: Unknown, y_pred: Unknown, from_logits: bool = False, label_smoothing: int = 0, axis: int = -1) -> Unknown) | ((y_true: Unknown, y_pred: Unknown, from_logits: bool = False, axis: int = -1) -> (Unknown | SparseTensor | IndexedSlices | Tensor | Any)) | ((y_true: Unknown, y_pred: Unknown, from_logits: bool = False, label_smoothing: int = 0, axis: int = -1) -> (Unknown | SparseTensor | IndexedSlices | Tensor | Any)) | None:
...
def cast_single_tensor(x, dtype=...): # -> SparseTensor | IndexedSlices | Tensor | Any:
  ...

def cast_if_floating_dtype_and_mismatch(targets, outputs): # -> SparseTensor | IndexedSlices | Tensor | Any | list[Unknown]:
...
def cast_if_floating_dtype(x, dtype=...): # -> defaultdict[Unknown, Unknown] | Any | list[Unknown] | ObjectProxy:
...
def cast_to_model_input_dtypes(x, model): # -> defaultdict[Unknown, Unknown] | Any | list[Unknown] | ObjectProxy:
...
def prepare_loss_functions(loss, output_names): # -> list[Unknown | Loss | Any | LossFunctionWrapper] | defaultdict[Unknown, Unknown] | Any | list[Unknown] | ObjectProxy:
...
def get_iterator(dataset): # -> OwnedIterator | Iterator:
...
def extract_tensors_from_dataset(dataset): # -> tuple[Unknown | Any | defaultdict[Unknown, Unknown] | ObjectProxy, Unknown | Any | None, Unknown | Any | None]:
...
class ModelInputs:
  ...
  def get_symbolic_inputs(self, return_single_as_list=...): # -> dict[str, None] | list[None] | None:
    """Returns inputs to be set as self.inputs for a model."""
    ...
  
  def as_dict(self): # -> Generator[tuple[str, None], None, None]:
...
def unpack_validation_data(validation_data, raise_if_ambiguous=...): # -> tuple[Iterator | IteratorBase | DatasetV2 | Sequence | Unknown, Unknown | None, Unknown | None]:
